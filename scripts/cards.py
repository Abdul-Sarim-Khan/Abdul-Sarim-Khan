...
def main(argv=None):
    ...
    try:
        ...
        print(f"  note: GitHub user/repos API unavailable ({e}), using defaults for preview", file=sys.stderr)
        user = {"public_repos": 4, "followers": 0}
        repos = []

    owned = [r for r in repos if not r["fork"]]
    stars = sum(r["stargazers_count"] for r in owned) if owned else 0
    ...
    for entry in wanted:
        ...
        repo_name = src["name"] if src else entry["repo"]
        card = {
            "name": repo_name,
